...
func SetupRouter() *gin.Engine {
	...
	// 获取野生水产品的上链信息
	r.POST("/getSeafoodInfo", con.GetSeafoodInfo)
	// 获取用户的野生水产品ID列表
	r.POST("/getSeafoodList", middleware.JWTAuthMiddleware(), con.GetSeafoodList)
	// 获取所有的keyword2
	r.POST("/getAllSeafoodInfo", middleware.JWTAuthMiddleware(), con.GetAllSeafoodInfo)
	// 获取野生水产品上链历史(溯源)
	r.POST("/getSeafoodHistory", middleware.JWTAuthMiddleware(), con.GetSeafoodHistory)
	return r
}
